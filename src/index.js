module.exports = function(layoutData, opts) {
  ...
  function genertePartJson(originJson) {
    ...
    function generateScript(json, options) {
      ...
      let propsScript = [].concat(
        _line('props: {', { indent: { tab: indent + 1 } }),
        _line(`dataId: {
            type: String,
            default: 'hm-imgcook-component'
          },
          option: {
            type: Object,
            default: {}
      ...
      let mountedFunctionScript = [].concat(
        _line('mounted() {', { indent: { tab: indent + 1 } }),
        _line(`if (this.props.data) {
            this.data = JSON.parse(JSON.stringify(this.props.renderData));
          }
